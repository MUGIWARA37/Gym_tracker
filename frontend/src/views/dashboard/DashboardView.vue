  ...
  <div>
    ...
    <div v-if="stats" class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <div class="rounded-xl p-4 card-life">
        ...
        <p class="text-2xl font-semibold text-white">{{ stats.weekly_sessions }}</p>
      </div>
      <div class="rounded-xl p-4 card-life">
        <p class="text-sm text-slate-500">Calories burned</p>
        <p class="text-2xl font-semibold text-white">{{ stats.weekly_calories_burned }}</p>
      </div>
      <div class="rounded-xl p-4 card-life">
        <p class="text-sm text-slate-500">Current streak</p>
        <p class="text-2xl font-semibold text-white">{{ stats.current_streak_days }} days</p>
      </div>
      <div class="rounded-xl p-4 card-life">
        <p class="text-sm text-slate-500">Goal completion</p>
        <p class="text-2xl font-semibold text-white">{{ stats.goal_completion_percent }}%</p>
      </div>
    </div>
  </div>
</template>
